#!/usr/bin/env python3
"""
Neolink Manager für WildCam
Generiert automatisch neolink.toml aus camera_config.json für Battery-Kameras (Port 9000)
"""
import json
import re
import sys
...
def parse_rtsp_url(rtsp_url):
    ...
    Hinweis: '#' in Passwörtern muss normalerweise URL-encoded (%23) sein.
...
def is_baichuan_camera(camera):
    """Prüfe ob Kamera Baichuan-Protokoll (Port 9000) nutzt."""
...
def generate_neolink_config(camera_config_path, output_path='neolink.toml'):
    ...
    try:
        ...
        print(f"❌ Fehler: {camera_config_path} nicht gefunden!")
        return False
    ...
    if not battery_cameras:
        print("ℹ️  Keine Battery-Kameras (Port 9000) gefunden.")
        print("   Neolink wird nicht benötigt.")
        return False
    
    print(f"🔋 {len(battery_cameras)} Battery-Kamera(s) gefunden:")
...
def update_camera_config(camera_config_path):
    ...
    if updated:
        ...
        try:
            ...
            print(f"✅ {camera_config_path} aktualisiert!")
            return True
        except Exception as e:
            print(f"❌ Fehler beim Speichern: {e}")
            return False
    else:
        print("ℹ️  Keine Updates nötig.")
        return False
...
def main():
    ...
    print("\n" + "=" * 50)
    print("📋 Schritt 2: Camera Config aktualisieren?")
    ...
    if update:
        ...
        print("ℹ️  Übersprungen. URLs bleiben unverändert.")
